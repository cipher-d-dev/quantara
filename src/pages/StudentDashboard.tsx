...
export function StudentDashboard() {
  ...
  const stats = [
    ...
    {
      label: 'Open Slots',
      value: availableCourses.filter((c) => c.registration_open && c.remaining_slots > 0).length,
      icon: TrendingUp,
      color: 'success',
    },
    {
  ...
  return (
    <DashboardLayout>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div ref={statsRef} className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            {stats.map(({ label, value, icon: Icon, color }) => (
              <div
                key={label}
                className="relative overflow-hidden rounded-2xl bg-surface-900/60 border border-surface-800/60 backdrop-blur-sm p-5 group hover:border-surface-700/80 transition-colors duration-200"
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${
                    ...
                    color === 'success' ? 'bg-success-500/10 text-success-400' :
                ...
                <div className={`absolute inset-x-0 bottom-0 h-px ${
                  ...
                  'bg-gradient-to-r from-transparent via-surface-700/40 to-transparent'
                }`} />
              </div>
            ))}
          </div>
          ...
          <div ref={contentRef} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2">
              <div className="rounded-2xl bg-surface-900/60 border border-surface-800/60 backdrop-blur-sm overflow-hidden">
                <div className="px-6 py-5 border-b border-surface-800/60">
                  <h2 className="text-base font-semibold text-surface-100">Available Courses</h2>
                ...
                <div className="p-6">
                  {coursesLoading ? (
                    ...
                    <div className="text-center py-14">
                      <div className="w-12 h-12 rounded-2xl bg-surface-800/60 flex items-center justify-center mx-auto mb-4">
                        <AlertCircle className="w-6 h-6 text-surface-600" aria-hidden="true" />
                      </div>
            ...
            <div>
              <div className="rounded-2xl bg-surface-900/60 border border-surface-800/60 backdrop-blur-sm overflow-hidden">
                <div className="px-6 py-5 border-b border-surface-800/60">
                  <h2 className="text-base font-semibold text-surface-100">My Registrations</h2>
                  <p className="text-xs text-surface-500 mt-1">Your active slot registrations</p>
                </div>
                <div className="p-4">
                  {registrationsLoading ? (
                    <div className="space-y-3">
                      <div className="h-20 bg-surface-800/40 rounded-xl animate-pulse" />
                      <div className="h-20 bg-surface-800/40 rounded-xl animate-pulse" />
                    </div>
                  ) : registeredCourses.length === 0 ? (
                    <div className="text-center py-10">
                      <div className="w-10 h-10 rounded-xl bg-surface-800/60 flex items-center justify-center mx-auto mb-3">
                        <Clock className="w-5 h-5 text-surface-600" aria-hidden="true" />
                      </div>
                    ...
                    <div className="space-y-2">
                      {registeredCourses.map((course) => (
                        <div
                          key={course.id}
                          className="p-4 rounded-xl bg-surface-800/40 border border-surface-700/40 hover:border-brand-500/30 transition-colors duration-200"
                        >
                          <div className="flex items-start justify-between mb-2">
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => { setSelectedCourse(course.id); setShowUnregisterModal(true); }}
                              className="text-error-500 hover:bg-error-500/10 -mr-1 -mt-1 text-xs"
                            >
                              Unregister
                            </Button>
                          </div>
                          <p className="text-sm font-medium text-surface-100 line-clamp-1 mb-2">{course.title}</p>
                          <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-surface-500">
                            ...
                            <span className={`capitalize font-medium ${course.paymentStatus === 'paid' ? 'text-success-400' : 'text-warning-400'}`}>
      ...
      <ConfirmModal
        ...
        onClose={() => { setShowUnregisterModal(false); setSelectedCourse(null); }}
        onConfirm={handleUnregister}
        title="Unregister from Course"
        description="Are you sure you want to unregister? Your slot will be made available to other students."
